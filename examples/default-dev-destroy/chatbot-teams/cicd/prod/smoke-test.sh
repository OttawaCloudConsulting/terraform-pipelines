...
#   - Run lightweight integration tests against PROD infrastructure
#   - Verify DNS records or certificate status
#
...
#   - AWS credentials are for the PROD target account (role already assumed)
#   - For dev-destroy variant: PROD tests passing triggers the DEV destroy stage
#   - Keep tests non-destructive — this is production
...
# Example: verify a resource exists in the PROD account
